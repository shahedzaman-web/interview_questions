      ...
      <div className="mb-8">
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="mt-1 text-gray-700">Node.js is an open-source, cross-platform runtime environment that executes JavaScript code on the server side. It is built on Chrome's V8 JavaScript engine and uses an event-driven, non-blocking I/O model that makes it lightweight and efficient.</p>
            <p className="mt-1 text-gray-700">Node.js works on a single-threaded, event-driven architecture using the V8 engine (compiles JS to machine code), the event loop (handles asynchronous tasks without blocking), and the libuv library (provides a thread pool for background tasks like file system and networking operations).</p>
          </div>
          <div>
            <strong>2. What is NPM and what does it stand for?</strong>
            <p className="mt-1 text-gray-700">NPM stands for Node Package Manager. It is used in Node.js to install and manage packages (libraries or dependencies) required for JavaScript applications. It uses package.json to manage project dependencies and metadata and works through CLI commands like npm install, npm update, and npm uninstall.</p>
          </div>
          <div>
            <strong>3. Why is Node.js single-threaded? How does it handle concurrency?</strong>
            <p className="mt-1 text-gray-700">Node.js is single-threaded because it is based on the asynchronous, non-blocking nature of JavaScript. This design makes it simpler to develop and maintain, allowing Node.js to handle many concurrent requests efficiently. It handles concurrency through its event-driven, non-blocking I/O model: I/O operations run asynchronously, so other tasks continue without blocking, and completed tasks add callbacks to a queue which the event loop executes.</p>
          </div>
          <div>
            <strong>4. What is the difference between synchronous and asynchronous functions?</strong>
            <p className="mt-1 text-gray-700">Synchronous functions block execution until the task completes and execute tasks sequentially. Asynchronous functions do not block execution; they allow other tasks to proceed concurrently, typically returning a promise or callback to handle the result upon completion.</p>
          </div>
          <div>
            <strong>5. What is the purpose of module.exports?</strong>
            <p className="mt-1 text-gray-700">module.exports is used to expose functions or variables of a particular module or file to be used elsewhere in the project. This helps encapsulate similar functions in a file, which improves project structure.</p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="mt-1 text-gray-700">Express.js is a lightweight and flexible web framework built on top of Node.js to create APIs easily. It is used because it simplifies routing, middleware handling, and request-response logic, making backend development faster and more structured. In the MERN stack, Express serves as the backend framework.</p>
          </div>
          <div>
            <strong>12. What is middleware in Express.js? Give examples of built-in and custom middleware.</strong>
            <p className="mt-1 text-gray-700">Middleware is a function that runs between the request coming in and the response going out. It is commonly used for tasks like authentication, logging, request validation, and error handling. Built-in middleware examples include express.json() (parses JSON bodies), express.static() (serves static files), and express.urlencoded() (parses URL-encoded bodies). Custom middleware is user-defined functions that can modify req/res objects, end the request-response cycle, or call next() to pass control to the next middleware.</p>
          </div>
          <div>
            <strong>13. How do you implement centralized error handling in Express?</strong>
            <p className="mt-1 text-gray-700">Centralized error handling is done using a special middleware function that takes four parameters (err, req, res, next) instead of three. Express identifies error-handling middleware by this function arity. Errors are passed to this middleware by calling next(err) from any route or middleware, and it sends a consistent error response to the client.</p>
