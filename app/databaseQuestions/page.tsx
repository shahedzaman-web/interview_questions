...
export default function DatabaseQuestions() {
  const dbmsBasics = [
    ...
    {
      q: "What is normalization and its normal forms (1NF, 2NF, 3NF, BCNF)?",
      a: "Normalization is the process of organizing data in a relational database to reduce redundancy, eliminate anomalies (insert, update, delete issues), and improve data integrity by splitting large tables into smaller related tables.<br /> 1NF ensures atomic values (no repeating groups).<br/> 2NF ensures the table is in 1NF and removes partial dependency (non-key attributes must depend on the full composite key).<br/> 3NF ensures the table is in 2NF and removes transitive dependency (non-key attributes must depend only on the primary key).<br/> BCNF is a stronger form of 3NF where every determinant must be a candidate key, eliminating all remaining functional dependency anomalies."
    },
